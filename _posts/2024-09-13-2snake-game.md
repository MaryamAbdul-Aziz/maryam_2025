...
let handleTouch = function(event) {
    ...
    const snakeHeadX = snake[0].x * BLOCK;
    const snakeHeadY = snake[0].y * BLOCK;

    // Determine the direction based on where the user tapped relative to the snake's head
    if (Math.abs(touchX - snakeHeadX) > Math.abs(touchY - snakeHeadY)) {
        // Left or Right
        if (touchX > snakeHeadX && snake_dir !== 3) {
            // Tap is to the right, and snake is not moving left
            snake_next_dir = 1; // Move Right
        } else if (touchX < snakeHeadX && snake_dir !== 1) {
            // Tap is to the left, and snake is not moving right
            snake_next_dir = 3; // Move Left
        }
    } else {
        // Up or Down
        if (touchY > snakeHeadY && snake_dir !== 0) {
            // Tap is below, and snake is not moving up
            snake_next_dir = 2; // Move Down
        } else if (touchY < snakeHeadY && snake_dir !== 2) {
            // Tap is above, and snake is not moving down
            snake_next_dir = 0; // Move Up
        }
    }
}
